...
function BootstrapCookieConsentSettings(props) {
    ...
    function showDialog() {
        Events.documentReady(function () {
            ...
            if (!self.modal) {
                ...
                // load content
                var templateUrl = self.props.contentURL + "/" + self.lang
                $.get(templateUrl)
                    .done(function (data) {
    ...
    function gatherOptions(setAllExceptNecessary) {
        ...
        const modal = $('.modal-dialog.cookies');
        options['lang'] = modal.data('lang');
        options['ulid'] = modal.data('cookie-ulid');
        return options
    }

    function agreeAll() {
        const settings = JSON.stringify(gatherOptions(true));
        Cookie.set(self.props.cookieName, settings, self.props.cookieStorageDays)
        self.$modal.modal("hide");
        saveSettingsOnServer(settings);
    }

    function doNotAgree() {
        const settings = JSON.stringify(gatherOptions(false));
        Cookie.set(self.props.cookieName, settings, self.props.cookieStorageDays)
        self.$modal.modal("hide");
        saveSettingsOnServer(settings);
    }
    ...
    function saveSettings() {
        const settings = JSON.stringify(gatherOptions());
        Cookie.set(self.props.cookieName, settings, self.props.cookieStorageDays)
        self.$modal.modal("hide");
        saveSettingsOnServer(settings);
    }
    ...
    function saveSettingsOnServer(settings){
        ...
        $.ajax({
            ...
            data: {
                settings:settings,
                ulid: modal.data('cookie-ulid'),
                lang: modal.data('lang')
            },
